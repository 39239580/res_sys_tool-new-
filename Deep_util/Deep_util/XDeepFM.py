...
import os
from Deep_util.xdeepfm_batch_next import BacthDataset
...
class XDeepFM(object):
    def __init__(self,
                 ...
                 cin_param=[12, 12],   # cin 部分，每个组件的行数
                 ...
                 regularaztion_rate_l2=0.0001,
                 regularaztion_rate_l1=0.0001,
                 # batch_norm_momentum =0.99,  # 批量标准层， 滑动栋梁
                 # batch_epsilon = 1e-3,  # 批量标准层的参数
        ...
        self.deep_layers_activattion = deep_layers_activattion   # 深度层的  激活函数
        ...
        self.regularaztion_rate_l1 = regularaztion_rate_l1
        self.regularaztion_rate_l2 = regularaztion_rate_l2
        # self.batch_norm_momentum = batch_norm_momentum   # 批量标准层， 滑动栋梁
        # self.batch_epsilon = batch_epsilon   # 批量标准层的参数
        ...
        self.regularizer = self._regularizers()   # 正则化参数
        self._init_graph()

    # 正则函数
    def _regularizers(self):
        if self.regularizer_type == "l2":
            regularizers = l2_regularizer(self.regularaztion_rate_l2)
        elif self.regularaztion_rate_l1 == "l1":
            regularizers = l1_regularizer(self.regularaztion_rate_l1)
        elif self.regularizer_type == "l1_l2":
            regularizers = l1_l2_regularizer(self.regularaztion_rate_l1, self.regularaztion_rate_l2)
        else:
            regularizers = l2_regularizer(self.regularaztion_rate_l2)
        return regularizers
    ...
    def _cin_net(self, bias=False):  # cin 网络结构
        ...
        for idx, layer_size in enumerate(self.cin_param):
            ...
            dot_result_hko = tf.reshape(dot_result_hkm,
                                        shape=[self.embedding_size, -1, field_nums[-1] * field_nums[0]])
            ...
            cin_second = tf.compat.v1.nn.conv1d(dot_result, filters=self._filter_cin(filter_param), stride=1, padding="VALID")
            # print(cin_second)
            ...
            if self.direct:
                ...
                if idx != len(self.cin_param) - 1:
                    # 非最后一层
                    next_hidden, direct_connect = tf.compat.v1.split(cin_unit, 2 * [int(layer_size / 2)],
                                                                     2)  # 取一半值，将D进行分解
                    # 一半用于下一层输入，一半丢给输出
    ...
    def _batch_norm_layer(self, x, train_phase, scope_bn):  # BN层
        ...
        # # 2.x版本
        # bn_train = batch_norm(x,momentum=self.batch_norm_momentum, epsilon=self.batch_epsilon,
        #                       center=True, scale=True,  trainable=True, name= scope_bn )
        #
        # bn_inference = batch_norm(x, momentum=self.batch_norm_momentum, epsilon=self.batch_epsilon,
        #                           center=True, scale=True, trainable=True, name= scope_bn)
    ...
    def evaluate(self, real_label):  # 评估模块
        """
        :param real_label:
        :return:
        """
        if self.evaluate_type == "acc" or self.evaluate_type == "all":
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.dropout_keep_cin = tf.compat.v1.placeholder(tf.float32, shape=[None],
                                                             name="dropout_keep_cin")  # None
    ...
    def train(self, data, save_params, every_save=1000, every_eval=1000, early_stop=False,
        ...
        with tf.compat.v1.Session(graph=self.graph, config=config) as sess:
            ...
            if train_test_split:  # 需要训练集与验证集分离操作
                xi_train, xi_valid, xv_train, xv_valiad, label_train, label_valid = data.train_test_split(
                    test_size=0.3)
                # 进行数据分离操作
            ...
            for epoch in range(self.traning_step):
                ...
                if train_test_split:  # 有验证集 则进行评估处理
                    ...
                    if epoch % every_eval == 0:  # 每隔一定轮数，进行操作
                        ...
                        cal_op = self.evaluate(real_label=label_v)
                        if self.evaluate_type == "all":
                            valid_loss, _, valid_acc = sess.run(cal_op, feed_dict=self.set_on_batch(xi_v, xv_v,
                                                                                                    label_v,
                                                                                                    False))

                            pred, label_v = self._predict(sess=sess, predict_data=valid_data, ifsplit=True)
    ...
    def _predict(self, sess, predict_data, ifsplit=False):  # 不能供外部使用
        """
        :param predict_data:
        ...
        if ifsplit:
            ...
            for eval_epoch in range(int(predict_num_example / self.batch_size)):
                predict_xi, predict_xv, predict_label = predict_data.next_batch(self.batch_size,
                                                                                list2multlist=False)
